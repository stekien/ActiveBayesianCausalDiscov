           ...
           # _ = plt.title("Objective for $D_2$")
            ...
            if (g_x2 < g_x1):
                ...
                try:
                    ...
                    X2_interv_x1 = np.array([self.fun(X1_interv_x1[0]) + rng.normal(loc=0.0, scale=np.sqrt(1), size=1)]).reshape(1,)
                    print('X1_interv_x1: ',X1_interv_x1, 'X2_interv_x1: ',X2_interv_x1)
                    # first intervention
                    self.pobInt_X1_givenF_G1(X1_interv_x1, X2_interv_x1)
                    self.pobInt_X1_given_G2(X1_interv_x1, X2_interv_x1)
...
def interv_x(x, X_train, Y_train):
    x_interv = x
    y_dox = np.array([f(x_interv) + rng.normal(loc=0.0, scale=np.sqrt(1), size=1)]).reshape(1,)
...
def evaluate(result):    
    ...
    success_80 = np.sum((result[:,0] >= 0.95)) / 1000
...
def evaluate1(result):    
    ...
    success_80 = np.sum((result[:,1] >= 0.95)) / 1000
...
def evaluate2(result):    
    ...
    success_80 = np.sum((result[:,2] >= 0.95)) / 1000
