# -*- coding: utf-8 -*-
"""
Created on Mon Apr 25 09:11:19 2022
...
noise_std = np.sqrt(0.7)
y_train_noisy = y_train + rng.normal(loc=0.0, scale=noise_std, size=y_train.shape)

#plt.style.use('seaborn')
plt.scatter(X_train, y_train_noisy, color="black", label=r"$Observations$")
plt.xlabel("$X$")
plt.ylabel("$Y$")
plt.legend()
plt.show()
...
k = ConstantKernel(1.0, constant_value_bounds="fixed") * RBF(length_scale=gamma_g1, length_scale_bounds="fixed")
...
"""
k2 = ConstantKernel(1.0, constant_value_bounds="fixed") * RBF(length_scale=gamma_g2, length_scale_bounds="fixed")
