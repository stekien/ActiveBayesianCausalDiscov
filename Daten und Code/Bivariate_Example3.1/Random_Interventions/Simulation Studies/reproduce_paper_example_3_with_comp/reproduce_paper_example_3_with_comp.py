# -*- coding: utf-8 -*-
"""
Created on Mon Aug  1 17:20:02 2022
...
gamma_g1 = 1.75
sigma_x_g1 = 1
sigma_y_g1 = np.sqrt(1)

gamma_g2 = 1.75
sigma_x_g2 = 1
sigma_y_g2 = np.sqrt(1)
...
for j in range(10000):
    ...
    for i in range(0,10):
        ...
        X_append = rng.normal(loc = 0.0, scale = 1, size=(1)).reshape(-1, 1)
        y_append = (2 * np.tanh(X_append)).reshape(1,) + rng.normal(loc=0.0, scale=noise_std, size=1)
        
        X_train = np.concatenate([X_train, X_append])  
        y_train_noisy = np.concatenate([y_train_noisy, y_append]) 
